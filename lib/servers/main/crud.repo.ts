...
> extends GraphRepository {
  ...
  async getOne({
    ...
    const options = {
      query: this
        .gql`${this.generateGQL("query", `${this.getOneQuery({ id, fragment })}`)}`, //`
      fetchPolicy: cache ? "cache-first" : "network-only",
    } as QueryOptions;
    ...
    if (data) {
      const options = {
        mutation: this.gql`${this.generateGQL(
          ...
          `($data: Create${this.apiName}Input!)` as any,
        )}`, //`
  ...
  async update({
    ...
    const options = {
      mutation: this.gql`${this.generateGQL(
        ...
        `($data: Update${this.apiName}Input!)` as any,
      )}`, //`
      variables: { data },
      fetchPolicy: "no-cache",
    } as MutationOptions;
